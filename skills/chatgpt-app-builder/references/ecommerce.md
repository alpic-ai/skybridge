...
`grep -rn "@todo" src`, then resolve every marker. Fill the server-wide items, then each tool: complete its `@todo`s and verify it before moving to the next.
...
### Server-wide (`src/server.ts`, plus `.env.template`)
...
- [ ] `name` / `version`.
- [ ] server `instructions`: adapt the server-wide prompt to the catalog.
