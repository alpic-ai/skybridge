...
**Migrating from Skybridge `1.x` to `2.x`?** → Read [migrate-to-v2.md](references/migrate-to-v2.md) first. v2 replaces the `McpServer` singleton with the `Skybridge` app and a per-request `handler`.
