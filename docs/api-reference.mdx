...
| [useDisplayMode](/api-reference/use-display-mode) | ✅ | ⚠️ | MCP Apps: no "modal" mode |
...
<Note>
**Apps SDK APIs not yet in Skybridge:** `notifyIntrinsicHeight` (report dynamic widget height), `requestClose` (close widget from UI), and `requestCheckout` (Instant Checkout / monetization) have no dedicated hooks. Use them via `window.openai` when running in ChatGPT if needed. See [Apps SDK (ChatGPT)](/fundamentals/apps-sdk#apps-sdk-apis-not-yet-supported-in-skybridge) for details.
</Note>
