...
The template works best with the `chatgpt-plugin-builder` skill, which drives the fill workflow below. The scaffolder installs it by default; to add it to an existing project:

```bash
npx skills add alpic-ai/skybridge --skill chatgpt-plugin-builder
```
...
Fill this ecom template following the chatgpt-plugin-builder skill's ecommerce reference. Start with phase 1 and ask me for everything you need.
</Prompt>
