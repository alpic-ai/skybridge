...
icon: "sparkles"
---
<img src="/images/ux.png" alt="UX Illustration" style={{ width: "100%", maxHeight: "40vh", marginTop:"-0.75rem", objectFit: "cover", borderRadius: "15px" }} />

An MCP App is a new kind of surface with its own UX principles. The view shares the screen with an ongoing conversation, on a canvas the host owns and can reshape at any moment. Skybridge surfaces that environment through hooks, so the view can read its context and adapt.
...
The same hook reports the host's color scheme as `theme`, `"light"` or `"dark"`. A view on its own palette looks pasted into the conversation; read `theme` and follow it.
...
- **PiP** floats above the conversation and stays open, which fits live or changing content. On mobile it coerces to fullscreen.
...
You don't need four devices to check this. The [DevTools](/test/devtools) environment inspector flips theme, display mode, locale, and device type live, so you can watch the view adapt on localhost, then confirm it in a real host through the [tunnel](/test/tunnel).

For overlay surfaces, see [`useRequestModal`](/api-reference/use-request-modal) (a confirmation on top of the view) and [`useRequestSize`](/api-reference/use-request-size) (let the view ask for a height).
