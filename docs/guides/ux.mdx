...
[`useViewport`](/api-reference/use-viewport) reports the room the host gives the view: `maxHeight`, and the `safeArea` insets that keep content clear of device notches and the chat composer.

```tsx views/carousel.tsx highlight={7,13}
import { useDisplayMode, useUser, useViewport } from "skybridge/web";
import { useToolInfo } from "../helpers.js"; // generated, type-safe from server schema
...
[`useUser`](/api-reference/use-user) reports the host's color scheme as `theme`, `"light"` or `"dark"`. A view on its own palette looks pasted into the conversation; read `theme` and follow it.
...
```tsx views/carousel.tsx highlight={8,17,20-22}
import { useDisplayMode, useUser, useViewport } from "skybridge/web";
import { useToolInfo } from "../helpers.js"; // generated, type-safe from server schema
...
[`useUser`](/api-reference/use-user) also reports the `locale` and the `userAgent`: the device type, and whether it supports `hover` and `touch`.

```tsx views/carousel.tsx highlight={6,18}
import { useDisplayMode, useUser, useViewport } from "skybridge/web";
import { useToolInfo } from "../helpers.js"; // generated, type-safe from server schema
