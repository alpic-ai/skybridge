...
You never declare your own server:

- Your server's origin is added to `connectDomains` and `resourceDomains`, so same-origin requests and assets work untouched.
- In development, the dev server's WebSocket origin is added to `connectDomains` for hot reload.
