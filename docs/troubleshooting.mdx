...
The hooks came from `skybridge/web` instead of the [generated `helpers.ts`](/api-reference/generate-helpers). The bare [hooks](/api-reference/overview#hooks) aren't typed against your [app](/api-reference/skybridge); only the generated ones carry your [tool names, inputs, and outputs](/api-reference/register-tool).
...
Type inference needs the [app](/api-reference/skybridge) to export its type and `helpers.ts` to consume it, with every tool registered by method chaining so it lands in `AppType`.

<CodeGroup>
```ts server.ts highlight={6-8,11}
...
## Mobile or desktop app logs cannot be accessed

When your app runs in the ChatGPT or Claude desktop or mobile app, the [view](/build/view) has no console you can open. Run the dev server through the [tunnel](/test/tunnel) and update `forwardConsole` in `vite.config.ts` to forward the view's logs to your terminal: see [Read View Logs on Mobile or Desktop Apps](/test/tunnel#read-view-logs-on-mobile-or-desktop-apps).
...
## Server crashes on deploy with `ENOENT` or a permission error

A local file write, a SQLite database, a JSON file, a log, failing on a path you wrote yourself. Most deploy targets give your server no durable local filesystem: Cloudflare Workers has none at all, and container platforms like Alpic are read-only outside the OS temp directory, which itself doesn't survive a redeploy or a scale event. See [Persist Server Data](/guides/data-persistence) for what to use instead of a local file.
