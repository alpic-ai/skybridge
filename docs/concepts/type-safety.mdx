---
title: Type Safety
description: "Get end-to-end TypeScript type inference from server to widget with generateHelpers."
---
...
## Typed `view.component`

The Vite plugin scans `src/views/` and generates `.skybridge/widgets.d.ts`. That file augments an internal `WidgetNameRegistry` interface, narrowing `WidgetName` from `string` to the union of your actual widget file names.

Result: `view.component` autocompletes and typos become compile errors.

```typescript
server.registerTool(
  {
    name: "search",
    view: { component: "searhc" }, // ← Type error: not in "search" | "details" | …
  },
  handler,
);
```
