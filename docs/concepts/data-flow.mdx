...
flowchart LR
    ...
    style Widget fill:#2563EB,stroke:#1e40af,color:#fff
...
Before diving into the data flow, understand the difference between plain tools and widget tools. Both use `registerTool`, differentiated by the optional `view` field:
...
// Widget tool: has UI, returns structured data
server.registerTool(
  {
    name: "chart",
    inputSchema: { /* ... */ },
...
If you use `fetch()` directly from widgets, you need to configure Content Security Policy (CSP) headers. The host blocks requests to domains not explicitly allowed. Add allowed domains to `view.csp.connectDomains` in your `registerTool` call. See [registerTool](/api-reference/register-tool).
</Warning>
