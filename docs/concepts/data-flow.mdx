...
Before diving into the data flow, understand the difference between plain tools and view tools. Both use `registerTool`, differentiated by the optional `view` field:
...
// View tool: has UI, returns structured data
server.registerTool(
  {
    name: "chart",
    inputSchema: { /* ... */ },
...
When the host calls one of your views, it returns `structuredContent` to hydrate the React component:
...
`useToolInfo` provides the initial hydration data and should not be used as a mutable data store. For persistent view state that survives re-renders, use [`useWidgetState`](/api-reference/use-widget-state).
</Note>
...
| Persist view state | `useWidgetState` | Survives re-renders |
...
- [useWidgetState API](/api-reference/use-widget-state) - Persistent view state
