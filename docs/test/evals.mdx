...
Some things are not a fact you can match on. Did the assistant explain the tool result? Did it stay inside the app's scope? `toPassJudgment` sends the conversation to a second model, with criteria written in plain English:
...
This matcher is the only one you `await`. The judge reads the whole conversation: every turn, and every tool call with its arguments and result. It runs at temperature 0 and answers pass or fail, with its reasoning. The reasoning is the failure message:
...
The judge runs on the chat's own model. Pass `model` to use another one, and `scope: "lastTurn"` to judge only the final assistant message:
