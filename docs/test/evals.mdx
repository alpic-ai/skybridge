...
| `toPassJudgment(criteria)` | a judge model reads the conversation and grades it against `criteria` |
...
Some of what makes an app good is not a fact you can match on: whether the assistant explained the tool result, stayed inside the app's scope, or asked the right clarifying question. `toPassJudgment` hands the conversation to a second model along with criteria written in plain English:
...
It is `await`ed, unlike the other matchers. The judge reads every user turn, assistant turn and tool call with its arguments and result, runs at temperature 0, and returns a pass or fail plus its reasoning, which lands in the failure message:
...
The judge runs on the chat's own model unless you give it another one, and `scope: "lastTurn"` narrows what it reads to the final assistant message when the criteria are about the answer itself:
...
A judgment is a live model call, so it costs money and is not reproducible the way a tool-call assertion is. Reach for it when no matcher can express the criteria, and keep the criteria narrow enough that a fail points at one thing. If the provider itself breaks, the assertion throws `judge unavailable: <error>` rather than reporting a fail, so a flaky provider is never read as an app regression.
