...
- **Modals** are host-driven: when a view calls [`useRequestModal`](/api-reference/use-request-modal), DevTools plays the host and renders it over the conversation with a close button. Closing it, or pressing Escape, returns the view to inline.
