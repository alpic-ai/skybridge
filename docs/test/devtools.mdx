...
- **Display modes** behave as they do in the host: inline in the conversation column, pip as a floating card, fullscreen as an overlay bounded to the work area so the sidebar and header stay visible.
...
- **One runtime**: the preview mocks the [MCP Apps](/resources/apps-sdk-and-mcp-apps#mcp-apps) runtime, which is what every host receives. Host behaviour outside the spec is not reproduced, and the conversation around a preview is a skeleton, not a working client.
