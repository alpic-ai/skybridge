...
<Tip>
Using [Auth0](/guides/auth-providers#auth0), [Clerk](/guides/auth-providers#clerk), [Descope](/guides/auth-providers#descope), [Stytch](/guides/auth-providers#stytch), [WorkOS](/guides/auth-providers#workos), or [any other OAuth provider](/guides/auth-providers#any-other-provider)? Connect it in one line instead of wiring this by hand. With a provider configured you can also [mix public and authenticated tools](#mix-public-and-authenticated-tools) by declaring each tool's `securitySchemes` — Skybridge enforces them for you, no middleware or handler guards needed.
</Tip>
