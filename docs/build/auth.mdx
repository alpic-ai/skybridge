...
icon: "key"
---
<img src="/images/auth.png" alt="Auth Illustration" style={{ width: "100%", maxHeight: "40vh", marginTop:"-0.75rem", objectFit: "cover", borderRadius: "15px" }} />
...
Hosts find your authorization server through standard metadata documents ([RFC 9728](https://datatracker.ietf.org/doc/html/rfc9728)). `mcpAuthMetadataRouter` serves them at `/.well-known/oauth-protected-resource`, so the host can perform the authentication flow.
...
Once the user is signed in, every request carries `Authorization: Bearer <token>`. `requireBearerAuth` validates it before any handler runs: missing, invalid, or expired tokens get a 401, insufficient scopes a 403.
