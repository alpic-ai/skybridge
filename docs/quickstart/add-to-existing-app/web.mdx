...
- **[`useWidgetState`](/api-reference/use-widget-state)**: Persist state across view renders
...
Here's a simple view using Skybridge hooks. Views live in `src/views/` and `export default` a React component — the Vite plugin auto-mounts them.
