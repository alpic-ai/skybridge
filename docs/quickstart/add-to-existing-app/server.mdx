...
Your existing tools, resources, and prompts will continue to work as before. With `skybridge/server`, `registerTool` gains an optional `view` field so you can bind a tool to a React view in a single call.

## Project structure

Skybridge uses a flat `src/` layout with views in `src/views/`:
...
A view's file name must match `view.component` in `registerTool` (e.g., `component: "my-view"` → `src/views/my-view.tsx`). See [registerTool](/api-reference/register-tool).
...
The plugin auto-mounts every file in `src/views/` — no `mountView` call needed.
...
In your server code, register the view with `registerTool` + `view`:
