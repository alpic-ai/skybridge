---
title: useUser
description: "Read the host's theme, locale, and device capabilities"
---
...
function Carousel({ products }: { products: Product[] }) {
  ...
  return (
    <div className={`${isMobile ? "grid-mobile" : "grid-desktop"} ${theme}`}>
