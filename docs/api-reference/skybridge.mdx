...
type SkybridgeConfig = Implementation &
  ServerOptions & {
    ...
    oauth?: OAuthConfig | ((config: Config) => OAuthConfig | Promise<OAuthConfig>);
    json?: JsonOptions;
    skills?: boolean;
  };
```
...
An [`OAuthConfig`](/api-reference/custom-provider#returns), usually from an [identity provider](/guides/auth-providers), as a value or as a function of the `setup` result. The branded providers are async and perform network discovery, so pass them as a function: it runs once at `run()`, never at module import. When set, it mounts the well-known OAuth metadata and bearer-token verification on `/mcp`.
