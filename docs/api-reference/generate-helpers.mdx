...
Your app's type, `typeof app`. Export it from `src/server.ts` with `export type AppType = typeof app`, then pass it here.

<Info>
Inference works only when `handler` chains `.registerTool()` calls on the server it receives and returns the result, so the tool types accumulate on `typeof app`.
</Info>
