...
description: "Create fully typed hooks with end-to-end inference from your MCP server, like tRPC for widgets."
---
...
Export your server type so it can be imported in your widget code:
...
```tsx
// src/views/search-voyage.tsx
import { useCallTool, useToolInfo } from "../helpers";

export default function SearchVoyage() {
  const { callTool, isPending } = useCallTool("search-voyage");
