...
A click in your view does not advance the conversation: the model only acts on conversation turns. `useSendFollowUpMessage` posts a message into the conversation as if the user had typed it, starting the next assistant turn.
...
<CardGroup cols={3}>
  <Card title="Communicating with the Model" icon="message-circle" href="/guides/communicating-with-model">
    Active messages and passive context sync from a view
  </Card>
