...
A modal overlay is host-owned: a [view](/build/view) cannot mount one itself. `useRequestModal` lets the view ask the host to open it as a modal, pass data into it, and read whether the modal is currently showing.
