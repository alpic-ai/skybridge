...
The main CLI is available as both `skybridge` and `sb` commands after installing `@anthropic-ai/skybridge` in your project.
...
This is the default command, so you can also run it as:
...
```bash
# Terminal 1: Start Skybridge with forwarded host support
