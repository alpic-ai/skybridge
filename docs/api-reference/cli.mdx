...
| `--plain` | Disable the interactive UI and stream raw server logs to stdout, so you can pipe them through a formatter (e.g. `skybridge dev --plain \| bunyan`). |

#### Formatting structured logs

If your server uses a structured JSON logger like [bunyan](https://github.com/trentm/node-bunyan) or [pino](https://github.com/pinojs/pino), the raw JSON output is hard to read in the interactive dev UI. Pass `--plain` to disable the UI and emit the server's stdout verbatim, then pipe it through the logger's formatter:
...
In `--plain` mode the server's stdout is forwarded as-is so the formatter only sees your log lines. Everything else — the banner, URLs, tunnel state, restart notices, and TypeScript errors — is written to stderr, so it stays visible without polluting the pipe.
