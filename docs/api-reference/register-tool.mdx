...
<Info>
**viewUUID**: Skybridge automatically injects a unique `viewUUID` into `_meta` for every tool with view call. This enables `useWidgetState` and `createStore` to persist state across re-renders in MCP Apps via `localStorage`. Any `_meta` you return is preserved alongside the injected `viewUUID`.
</Info>
