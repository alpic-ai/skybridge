...
All three are the tool's prompt surface: the model reads them to decide when to call it, so write each for the model, not just `description`. `name` is also the identifier the call uses, and `title` a short display name.

### `inputSchema`, `outputSchema`

[Zod](https://zod.dev/) raw shapes. `inputSchema` validates the call and types the handler's `input`, and a field's `.describe()` text reaches the model. `outputSchema` types `structuredContent` and tells the model the shape to expect back.
...
<Info>
The `openai/*` keys are read only on **ChatGPT**.
</Info>

## Handler

Runs when the tool is called. It receives the validated `input` (typed from [`inputSchema`](#inputschema-outputschema)) and the request context `extra`, and returns the result fields below.
...
<CardGroup cols={3}>
  ...
  <Card title="FileRef" icon="paperclip" href="/api-reference/file-ref">
