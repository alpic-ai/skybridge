---
title: registerTool
description: "Register an MCP tool — with or without a React widget — in a single unified call."
---


Register an MCP tool on the server. Provide a `view` to bind the tool to a React widget; omit `view` for a plain tool with no UI.
...
A single call replaces the previous `registerTool(name, config, handler)` + `registerWidget(name, resourceConfig, toolConfig, handler)` pair.
...
type UnifiedToolConfig = {
  ...
  view?: ViewConfig;         // ← present = widget-tool, absent = plain tool
  _meta?: ToolMeta;
};
...
Binds the tool to a React widget.

```typescript
type ViewConfig = {
  component: WidgetName;                   // File name in src/views/ (typed — see below)
  description?: string;                    // Shown during widget discovery
  hosts?: ("apps-sdk" | "mcp-app")[];      // Defaults to both
  prefersBorder?: boolean;                 // Removes the default iframe border when false
  domain?: string;                         // Custom widget domain
  csp?: {
...
<Info>
**viewUUID**: Skybridge automatically injects a unique `viewUUID` into `_meta` for every widget tool call. This enables `useWidgetState` and `createStore` to persist state across re-renders in MCP Apps via `localStorage`. Any `_meta` you return is preserved alongside the injected `viewUUID`.
</Info>
...
<Tip>
If your widget makes client-side API calls, add those domains to `connectDomains`. Server-side calls from the handler do not require CSP configuration.
</Tip>
...
The Vite plugin generates `.skybridge/widgets.d.ts` at dev/build time, augmenting the internal `WidgetNameRegistry` interface. This narrows `WidgetName` from `string` to the union of actual widget file names in `src/views/`.

Upshot: `view.component` autocompletes, and typos become compile-time errors.

```typescript
view: {
  component: "greting", // ← Type error: not a valid WidgetName
}
```
...
Widgets run in a sandboxed iframe with strict Content Security Policy. If your widget needs to:
