...
<Info>
With an [`oauth`](/api-reference/mcp-server#constructor) provider configured, Skybridge enforces `securitySchemes` for you: anonymous or under-scoped calls are rejected before the handler runs, and any tool declaring a `noauth` scheme flips the server into mixed mode. Declaring no schemes falls back to the secure default (sign-in required).
</Info>

<Warning>
If you wire auth by hand instead (`mcpAuthMetadataRouter` + [`requireBearerAuth`](/api-reference/require-bearer-auth) / [`optionalBearerAuth`](/api-reference/optional-bearer-auth)), `securitySchemes` is client-facing metadata only, not self-enforcing: enforce each tool's own requirements in the handler from `extra.authInfo`.
</Warning>
