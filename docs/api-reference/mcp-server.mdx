...
Register a tool — with or without a React view — using a single unified call. Provide `view` to bind the tool to a view; omit it for a plain tool. See [registerTool](/api-reference/register-tool) for the full API.
