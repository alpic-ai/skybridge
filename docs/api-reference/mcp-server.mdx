...
`skills` serves [Agent Skills over MCP](/guides/skills). `true` serves skills from `src/skills`; a string sets a custom directory (dev only — production builds snapshot `src/skills`). Its presence gates the feature and declares the `io.modelcontextprotocol/skills` capability. See the [Skills guide](/guides/skills).
