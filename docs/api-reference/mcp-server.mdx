...
To extract the handler into its own declaration, annotate its parameter with `SkybridgeServer`, the bare server before any registration. It takes the claims your OAuth verifier produces, which types `extra.http.authInfo.extra` in tool handlers; when `oauth` is set on the same config, the claims are inferred and the annotation is unnecessary.
...
export const app = new Skybridge({ name: "shop", version: "1.0", handler });
```
