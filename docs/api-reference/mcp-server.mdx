...
`skills` serves [Agent Skills over MCP](/guides/skills). `true` enables the feature and declares the `io.modelcontextprotocol/skills` capability; pass `{ dir }` to override the skills directory (defaults to `src/skills`, dev only — production builds snapshot `src/skills`). See the [Skills guide](/guides/skills).
