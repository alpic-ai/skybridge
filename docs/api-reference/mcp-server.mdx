...
| `options` | `McpServerOptions` | Configuration options forwarded to the MCP SDK |
| `skybridgeOptions` | `SkybridgeServerOptions` | Skybridge-specific options |
...
The `json` field is forwarded to the [`express.json()`](https://expressjs.com/en/5x/api/express/#expressjsonoptions) body parser that Skybridge pre-applies to the underlying Express app. Use it to tune parsing — for example, to raise the default 100kb body size limit:
...
`express.json()` is pre-applied — tune it via the [constructor's `skybridgeOptions.json`](#skybridge-options). Register your handlers before `run()`; after `run()`, Skybridge appends dev-mode middleware (in development), the `/mcp` route, and the default error handler in that order.
