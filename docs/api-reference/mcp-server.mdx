...
`McpServer` is the root of your app: an Express-backed server whose methods return the server itself, so you chain your tool registrations off it and export the resulting type for [`generateHelpers`](/api-reference/generate-helpers).
...
type SkybridgeServerOptions = {
  json?: JsonOptions; // options for the built-in express.json() body parser, e.g. { limit: "10mb" }
};
```
...
The underlying [Express](https://expressjs.com/) app, for custom routes, middleware, and settings. Register handlers before `run()`.
...
Registers an [Express error handler](https://expressjs.com/en/guide/error-handling.html), optionally path-scoped, to run after the `/mcp` route. A default handler runs last, responding with a 500 JSON-RPC error when nothing else has sent a response.
