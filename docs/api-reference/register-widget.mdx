...
Async function that executes when the tool is called. Receives the validated input and an `extra` object with request context:
...
The `extra` parameter provides access to:
- `extra.authInfo` — Authentication info populated by auth middleware (e.g. `authInfo.clientId`, `authInfo.extra.email`)
...
Access authentication info from the `extra` parameter (requires auth middleware via [`.use()`](/api-reference/mcp-server#use)):
