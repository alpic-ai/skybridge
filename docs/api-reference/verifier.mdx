...
`verifyAccessToken` validates the token however your provider requires, returns an `AuthInfo` for a valid token, and throws `OAuthError` otherwise. Pass it to the middleware as `verifier: { verifyAccessToken }`.
...
- Resolve with an `AuthInfo` for a valid token. The middleware puts it on `extra.http?.authInfo` for tool [handlers](/api-reference/register-tool#handler).
- Throw `OAuthError("invalid_token", message)` for a malformed, badly signed, or expired token. The middleware returns a 401 with the right `WWW-Authenticate` header.
