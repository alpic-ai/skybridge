...
<Info>
Every target below runs your server in an environment with no durable local filesystem: some have none at all, others reset it on redeploy or on scale. If your tools read or write data that needs to outlive a single request, see [Persist Server Data](/guides/data-persistence) before you deploy.
</Info>
