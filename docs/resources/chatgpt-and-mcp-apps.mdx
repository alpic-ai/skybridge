...
<Hero src="/images/mcp-apps.webp" alt="ChatGPT and MCP Apps" />

MCP Apps is the open contract for rendering interactive views inside an AI client. ChatGPT implements it and adds `window.openai` on top: ChatGPT-specific APIs and compatibility aliases for its own capabilities. Skybridge targets the MCP Apps baseline and reaches for `window.openai` only where you opt into a ChatGPT feature, so one server and one set of views run on both.
...
## ChatGPT (`window.openai`)

ChatGPT implements MCP Apps and adds `window.openai` on top: ChatGPT-specific APIs and compatibility aliases for capabilities such as [file params](/api-reference/file-ref) and the [open-in-app URL](/api-reference/use-set-open-in-app-url). OpenAI's guidance is to use the MCP Apps `ui/*` bridge by default and reach for `window.openai` only for those extras. The [plugin reference](https://developers.openai.com/plugins/reference) documents the surface, and [Add UI to your MCP server](https://developers.openai.com/plugins/build/chatgpt-ui) covers how the two fit together.
