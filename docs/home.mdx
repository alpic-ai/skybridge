...
It comes with:
- 🌐 Write Once, Run Everywhere — Write once, run anywhere. It workly seamlessly with ChatGPT (Apps SDK) and MCP-compatible clients.
...
- **`skybridge/devtools`** : A dev environment, with a a local emulator and dev tools, and a Vite-based dev server allowing Hot Module Reload directly inside ChatGPT or MCP Client.
...
- **it is NOT an agentic framework.** It allows you to build ChatGPT and MCP Apps that run inside ChatGPT and MCP Clients. It's doesn't provide any agentic frontend to run your App.
