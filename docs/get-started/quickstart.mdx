...
- a working [MCP server](/api-reference/skybridge)
...
[Tools](/build/tools) are defined in `src/server.ts`, which `src/index.ts` runs, and [UI components](/build/view) live in the `views` directory. Tools are bound to views in the tool definition.
...
You'll be provided a public URL (e.g., `https://superb-marmot-fondue-420.alpic.dev/mcp`) to [register your server as a ChatGPT plugin or a Claude connector](/test/tunnel#connect-to-hosts).
