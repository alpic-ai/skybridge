...
> **ChatGPT only.** This example relies on [`useFiles()`](https://docs.skybridge.tech/api-reference/use-files) (`upload`, `selectFiles`, `getDownloadUrl`) and `openai/fileParams`, which are part of the ChatGPT Apps SDK and throw on other MCP hosts. Run and test it in ChatGPT.
...
- **Server-side ZIP Creation**: Builds ZIP archives with [`fflate`](https://github.com/101arrowz/fflate) (`zipSync`), since Node's `zlib` only does the raw deflate stream, not the ZIP container
- **Cloudflare R2 Storage**: Uploads generated archives to R2 and returns presigned download URLs, signed with [`aws4fetch`](https://github.com/mhart/aws4fetch) (required — the server won't start without R2 credentials)
...
```
│   ├── src/
│   │   ├── server.ts     # Server entry point (zip-file tool, fflate zipping)
│   │   ├── r2.ts         # Cloudflare R2 upload & presigned URLs (aws4fetch)
│   │   ├── env.ts        # .env loader
│   │   ├── views/
│   │   │   └── zip-file.tsx
...
Modify files in `src/` (e.g. `src/server.ts`) and refresh the connection with your testing MCP Client to see the changes.

### Testing your App

You can test your App locally by using our DevTools UI on `http://localhost:3000` while running the dev command.

Because it uses the ChatGPT file APIs, test the end-to-end flow in ChatGPT rather than Claude or VSCode. See [Testing Your App](https://docs.skybridge.tech/quickstart/test-your-app).
...
[![Deploy it on Alpic](https://assets.alpic.ai/button.svg)](https://app.alpic.ai/new/clone?repositoryUrl=https://github.com/alpic-ai/skybridge&rootDir=examples/chatgpt-files)
