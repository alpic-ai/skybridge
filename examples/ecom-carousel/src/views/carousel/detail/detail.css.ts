...
export const detail = style({
  ...
  // A fullscreen, text-heavy page owns its background so content.intense text
  // stays readable regardless of the host backdrop (the inline carousel stays
  // transparent via ViewFrame; only this fullscreen page paints a surface).
  minHeight: "100dvh",
  backgroundColor: colors.surface.light,
});

// Product / variant id, idiomatically top-right.
...
// D1/D3: rating + badges row directly under the title.
...
// D5: product facts as a simple list, one line per fact "label: value" (or just
...
// Primary CTA (magenta fill, white label in both themes).
