...
export function DetailView({ product }: { product: Product }) {
  ...
  // Custom Meta fields (variant-first, then card): rating by the title (D1),
  // badges as chips by the title.
  const rating = shown?.rating ?? product.card.rating;
  const reviewCount = shown?.reviewCount ?? product.card.reviewCount;
  const badges = shown?.badges ?? product.card.badges ?? [];
        ...
        <div className={styles.info}>
          ...
          {rating != null || badges.length > 0 ? (
            <div className={styles.meta}>
              {rating != null ? (
                <span className={cx(text({ style: "bodyS" }), styles.rating)}>
              ...
              {badges.map((b) => (
                <span
                  key={b}
                  className={cx(text({ style: "labelS" }), styles.badge)}
                >
                  {b}
                </span>
              ))}
