...
This "Murder in the Valley" example is an interactive murder mystery game that demonstrates key Skybridge capabilities:
- **Interactive Widget Rendering**: A React-based widget that displays a multi-screen game directly in AI conversations
- **Fullscreen Display Mode**: Switches to immersive fullscreen via `useDisplayMode()` hook
- **Follow-Up Messages**: Sends user choices back to the AI for dynamic responses via `useSendFollowUpMessage()` hook
- **Hot Module Replacement**: Live reloading of widget components during development

## Live Demo
Try it for yourself: `https://murder-in-the-valley.skybridge.tech/mcp`
